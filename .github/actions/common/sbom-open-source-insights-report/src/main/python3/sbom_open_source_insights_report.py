...
def parse_cyclonedx_1_5_sbom_report(cyclonedx_sbom_filename: str, csv_filename: str) -> dict:
    """Parse CycloneDX SBOM v1.5 JSON report

    :parameter
        cyclonedx_sbom_filename:str -- Name of CycloneDX SBOM JSON report to parse
        csv_filename:str -- Name of CSV report to write

    :return
        dict -- Parsed CycloneDX SBOM JSON report
    """
    log.info(f"Parsing CycloneDX SBOM v1.5 JSON report: {cyclonedx_sbom_filename}")
    ...
    try:
        ...
        for component in get_json_value(data, "components"):
            ...
            if component_group not in exclude_component_groups:
                # CycloneDX SBOM data
                component_scope = get_json_value(component, "scope")
                ...
                # Get CycloneDX SBOM v1.5 data for CSV report
                cyclonedx_sbom_data = [
                    bom_format,
                    spec_version,
                    component_scope,
                ...
                log.info(f"Fetched component: {component_group}/{component_name}@{component_version}")
        ...
        log.error(f"Error parsing CycloneDX SBOM v1.5 JSON report: {e}")
        sys.exit(1)
    else:
        log.info("Successfully parsed CycloneDX SBOM v1.5 JSON report")
    return None


def parse_cyclonedx_1_6_sbom_report(cyclonedx_sbom_filename: str, csv_filename: str) -> dict:
    """Parse CycloneDX SBOM v1.6 JSON report

    :parameter
        cyclonedx_sbom_filename:str -- Name of CycloneDX SBOM v1.6 JSON report to parse

    :return
        dict -- Parsed CycloneDX SBOM v1.6 JSON report
    """
    log.info(f"Parsing CycloneDX SBOM v1.6 JSON report: {cyclonedx_sbom_filename}")
    with open(cyclonedx_sbom_filename, "r") as file:
        data = json.load(file)
    ...
    try:
        ...
        for component in get_json_value(data, "components"):
            ...
            if "version" in component:
                ...
                # Get CycloneDX SBOM v1.6 data for CSV report
                cyclonedx_sbom_data = [
                    bom_format,
                    spec_version,
                    component_scope,
        ...
        log.error(f"Error parsing CycloneDX SBOM v1.6 JSON report: {e}")
        sys.exit(1)
    else:
        log.info("Successfully parsed CycloneDX SBOM v1.6 JSON report")
    return None
...
def get_csv_column_headers(programming_language: str) -> str:
    """Get column headers for CSV report

    :parameter
        programming_language:str -- Programming language of project
    ...
    if programming_language.upper() == "JAVA":
        # CycloneDX SBOM v1.5
        cyclonedx_sbom_headers = [
            "BOM Format",
            "Spec Version",
    ...
    elif programming_language.upper() == "PYTHON" or programming_language.upper() == "JAVASCRIPT":
        # CycloneDX SBOM v1.6
        cyclonedx_sbom_headers = [
            "BOM Format",
            "Spec Version",
...
def write_csv_report_header(csv_output_filename: str, programming_language: str) -> None:
    ...
    with open(csv_output_filename, "w") as file:
        file.write(get_csv_column_headers(programming_language))
...
def main(args: argparse.Namespace) -> None:
    ...
    """
    # TODO: Migrate Java to CycloneDX SBOM v1.6 after completion of https://github.com/CycloneDX/cyclonedx-maven-plugin/issues/489
    if args.programming_language.upper() == "JAVA":
        csv_output_filename = "sbom_open_source_insights_java_report.csv"
        write_csv_report_header(csv_output_filename, args.programming_language)
        parse_cyclonedx_1_5_sbom_report(args.cyclonedx_sbom_filename, csv_output_filename)
    elif args.programming_language.upper() == "PYTHON" or args.programming_language.upper() == "JAVASCRIPT":
        csv_output_filename = f"sbom_open_source_insights_{args.programming_language.lower()}_report.csv"
        write_csv_report_header(csv_output_filename, args.programming_language)
        parse_cyclonedx_1_6_sbom_report(args.cyclonedx_sbom_filename, csv_output_filename)
    else:

...
class TestSbomOpenSourceInsightsReport(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cyclonedx_sbom_open_source_insights_report.log = logging.getLogger()
        cyclonedx_sbom_open_source_insights_report.log.setLevel(logging.INFO)
    ...
    def test_query_osv_api(self, mock_response):
        osv_data = cyclonedx_sbom_open_source_insights_report.query_osv_api([], "pkg:pypi/django@5.0", "5.0")
    ...
    def test_get_json_value_missing_key(self):
        ...
        result = cyclonedx_sbom_open_source_insights_report.get_json_value(
            osv_data["vulns"][0], "published", "missing_key"
        )
    ...
    def test_get_vulnerability_cve(self):
        with open(RESPONSE_OSV_API_JAVA, "r") as f:
            osv_data = JSON.load(f)
        result = cyclonedx_sbom_open_source_insights_report.get_vulnerability_cve(osv_data["vulns"][0])
    ...
    def test_get_vulnerability_cve_not_found(self):
        with open(RESPONSE_OSV_API_PYTHON, "r") as f:
            osv_data = JSON.load(f)
        result = cyclonedx_sbom_open_source_insights_report.get_vulnerability_cve(osv_data["vulns"][1])
        self.assertEqual(result, "N/A")

    def test_get_vulnerability_affected_version_not_found(self):
        with open(RESPONSE_OSV_API_PYTHON, "r") as f:
            osv_data = JSON.load(f)
        result = cyclonedx_sbom_open_source_insights_report.get_vulnerability_affected_version(
            osv_data["vulns"][1], "1.0"
        )
    ...
    def test_java_main(self, mock_response):
        ...
        result = cyclonedx_sbom_open_source_insights_report.main(args)
